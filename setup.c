...
/*   Updated: 2019/08/26 11:47:49 by rjeor-mo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_fractal_number(char *s)
{
	...
	ft_strcpy(fractals[8], "MultiBC");
	ft_strcpy(fractals[9], "Julia");
	ft_strcpy(fractals[10], "MultiJ");
	ft_strcpy(fractals[11], "JuliaC");
	ft_strcpy(fractals[12], "MultiJC");
	i = 0;
...
void		get_funcs(void *(**f_funcs)(void*))
{
	...
	f_funcs[5] = trd_func_cship;
	f_funcs[6] = trd_func_ctricorn;
	f_funcs[7] = trd_func_cmand;
	f_funcs[8] = trd_func_cmultibrot;
	f_funcs[9] = trd_func_julia;
	f_funcs[10] = trd_func_multijulia;
	f_funcs[11] = trd_func_cjulia;
	f_funcs[12] = trd_func_cmultijulia;
}
...
void		set_controls(int fractal_num, t_param *param)
{
	if (fractal_num < 9)
	{
		...
		if (fractal_num >= 5 && fractal_num <= 8)
			param->is_chess = 1;
	}
	else if (fractal_num >= 9)
	{
